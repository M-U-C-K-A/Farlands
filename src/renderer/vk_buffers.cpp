...
void VkBufferManager::init(VkContext &ctx,
  ...
  createDescriptorSets(ctx.getDevice(), descriptorSetLayout);
}
...
void VkBufferManager::updateMeshBuffers(VkContext &ctx,
  ...
  copyBuffer(ctx.getDevice(), ctx.getGraphicsQueue(), vStaging, m_vertexBuffer,
             vSize);
  ...
  createBuffer(ctx, iSize,
               ...
               m_indexBufferMemory);
  copyBuffer(ctx.getDevice(), ctx.getGraphicsQueue(), iStaging, m_indexBuffer,
             iSize);
