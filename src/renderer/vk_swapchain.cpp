...
void VkSwapchain::createFramebuffers(VkDevice device,
...
// ── Image Views ─────────────────────────────────────────────────
VkImageView VkSwapchain::createImageView(VkDevice device, VkImage image,
                                         VkFormat format,
...
void VkSwapchain::createImageViews(VkDevice device) {
  ...
  for (size_t i = 0; i < m_images.size(); i++)
    m_imageViews[i] =
        createImageView(device, m_images[i], m_imageFormat,
...
void VkSwapchain::createDepthResources(VkContext &ctx) {
  ...
  createImage(ctx, m_extent.width, m_extent.height, depthFormat,
              VK_IMAGE_TILING_OPTIMAL,
...
VkExtent2D
VkSwapchain::chooseSwapExtent(const VkSurfaceCapabilitiesKHR &caps,
                              GLFWwindow *window) {
