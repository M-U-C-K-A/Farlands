...
void Renderer::init(GLFWwindow *window) {
  ...
  m_buffers.init(m_context, m_pipeline.getDescriptorSetLayout());
...
void Renderer::createCommandBuffers() {
  ...
  if (vkCreateCommandPool(m_context.getDevice(), &cpci, nullptr,
                          &m_renderCommandPool) != VK_SUCCESS)
  ...
  if (!inMenu) {
    ...
    if (m_buffers.getIndexCount() > 0 &&
      ...
      vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
                              m_pipeline.getPipelineLayout(), 0, 1, &ds, 0,
                              nullptr);
      vkCmdDrawIndexed(cmd, m_buffers.getIndexCount(), 1, 0, 0, 0);
    }
  }
...
void Renderer::drawFrame(const UniformBufferObject &ubo, bool inMenu) {
  ...
  if (result == VK_ERROR_OUT_OF_DATE_KHR) {
    m_swapchain.recreate(m_context, m_window, m_pipeline.getRenderPass());
    return;
  }
