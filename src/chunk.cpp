    ...
    for (int z = 0; z < CHUNK_SIZE_Z; ++z) {
        for (int y = 0; y < CHUNK_SIZE_Y; ++y) {
            for (int x = 0; x < CHUNK_SIZE_X; ++x) {
                ...
                if (type == BlockType::Grass) color = colorGrassFront;
                else if (type == BlockType::Stone) color = colorStone;

                if (getBlock(x, y, z + 1) == BlockType::Air) addFace(x, y, z, 0, color);
                if (getBlock(x, y, z - 1) == BlockType::Air) addFace(x, y, z, 1, color);
                if (getBlock(x, y + 1, z) == BlockType::Air) {
                    glm::vec3 topColor = (type == BlockType::Grass) ? colorGrassTop : color;
                ...
                if (getBlock(x + 1, y, z) == BlockType::Air) addFace(x, y, z, 4, color);
                if (getBlock(x - 1, y, z) == BlockType::Air) addFace(x, y, z, 5, color);
            }
        }
    }
}
